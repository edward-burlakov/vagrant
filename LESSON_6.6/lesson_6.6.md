...
2) Также можно использовать команду  db.currentOp(),  
   ...
   - При отсутствии кластера необходимо запустить  команду на ноде с запущенным демоном mongod
                 ...
                 где db1 - имя БД ; 180 - время в секундах)

   -  Если мы имеем кластерное решение с шардами , то можно использовать двухэтапную агрегацию : 
      Чтобы получить результат, относящийся только к этому запросу клиента ,запущенному на этом экземпляре mongos
      команду агрегации необходимо запускать  именно на данной ноде ,гле клиент запустил запрос.
