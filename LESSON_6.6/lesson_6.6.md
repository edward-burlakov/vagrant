...
8) Исправление ситуации:
   ...
   Предполагаемые  причины две:
       ...
       1) Работа кластера нарушена ( например, мастер-нода пытается и не может сохранить изменнёные ключии на удаленные реплики и на это время блокирует  новые запасиси в БД) 
        То есть, согласно CAP-теореме, кластер работает в режиме AP.
...
4) Еще вариант - изменить поведение ядра таким образом, чтобы оно не « перехватывало » память. 
