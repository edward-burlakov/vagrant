...
Причина - в недостатке оперативной памяти на хосте, в результате ОС c помощью процесса OOM-killer завершает процесс,
максимально утилизирующий память, чтобы предотвратить падение всей системы.
...
2) Можно также исключить процесс PostgreSQL из расстрельного списка OOO "Киллер". 
