...
1) Обычно это указывает на проблемы с подключением к сети, и вам следует проверить состояние вашей сети, если эта ошибка возникает часто. 
...
3) Реже это может произойти, когда клиент пытается установить первоначальное соединение с сервером. В этом случае, 
   ...
   Вы можете увидеть « reading authorization packet » как часть сообщения об ошибке; если это так, это также говорит о том, 
   что это решение, которое вам нужно.

- Если причина не является ни одной из только что описанных, у вас может возникнуть проблема со BLOB-значениями, 
   превышающими параметр "max_allowed_packet", что может вызвать эту ошибку на некоторых клиентах. 
...
1) Для предупреждения убийства  процесса, для начала необходимо увеличить объем ОЗУ 
    или выставить ограничение в параметрах настройки PostgreSQL на использование ОЗУ хоста - 
    -max_connections, 
    -shared_buffer, 
    -work_mem, 
...
2) Если возможности добавить ОЗУ  на хост нет, то Можно также исключить процесс PostgreSQL из расстрельного списка OOO "Киллер". 
       ...
       - Сначала выясним идентификатор (PID) процесса postmaster:
        ...
        - Затем настраиваем  OOM Killer так, чтобы он  и не думал убить процесс с PID 3234 — записываем в файл oom_adj 
          ...
          Можно это сделть путем помещения данной операции перед командой  запуска БД PostgreSQL в срипте инициализации.  
...
3) Еще вариант - изменить поведение ядра таким образом, чтобы оно не « перехватывало » память. 
   Хотя этот параметр не предотвратит вызов OOO "Киллер", он значительно снизит эту вероятность и,
