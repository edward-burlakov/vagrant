...
#### Высоконагруженное монолитное java веб-приложение;
     - Рекомендую физический сервер.  
       Предполагается монолитная компоновка - следовательно микросервисная архитектура не применима без изменения кода. 
       Предполагается  высокая нагрузка  и отсутствие масштабирования -  значит необходим скоростной физический доступ к ресурсам (CPU и дискам).
       Виртуалзация с гипервизором или физический сервер без использования слоя гипервизора.
---
#### Nodejs веб-приложение;
    - Эта платформа на основе JavaScript для серверных и сетевых приложений - чистое веб-приложение, 
      поэтому рекомендую контейнеризацию в докере. 
      Контейнеризация позволит быстро запустить приложение в работу, включая  необходимые библиотеки.
---  
#### Мобильное приложение c версиями для Android и iOS;
    - Непонятно на чем реализована серверная часть приложения, а именно на какой платформе и языке. 
      Наиболее вероятно, что используемый тип платформы не имеет подходящих образов docker-контейнеров. 
      Хотя с точки зрения масштабируемости ( огромное число пользователей) контейнеризация очень привлекательна. 

---
#### Шина данных на базе Apache Kafka;
      Распредеелённая потоковая платформа  для постороения конвейеров в режиме реального времени, которая может использоваться как брокер сообщений.
    - Количество реализаций в контейнерах просто зашкаливает - значит  решение успешно.
      Также возможна и виртуализация. Выбор зависит от передаваемого трафика и типа сервера (тест-сервер или боевой сервер ).
...
#### Elasticsearch кластер для реализации логирования продуктивного веб-приложения - три ноды elasticsearch, два logstash и две ноды kibana;
    - Ноды Elasticsearch лучше развернуть на виртуальных хостах внутри отказоустойчивого кластера, например VMWARE VCenter,
      хотя количестов реализаций на Dokcer-образе  - огромно.
      Все ноды kibana ( средство визуализации) и logstash ( бесплатный парсинг логов) - развернуть на  docker-контейнерах или в виртуальную среду, 
      т.к. существуют реализации в обоих исполнениях. 
   
---
#### Мониторинг-стек на базе Prometheus и Grafana;
       Prometheus - система мониторинга. Контейнеризация подходит. С единственной оговоркой -если вы удалите контейнер, 
           все ваши данные будут потеряны,а при следующем запуске образа база данных будет повторно инициализирована. 
           Чтобы избежать этой потери данных, вы должны смонтировать том на внешней системе.
           Что касается демона node_exporter  - его необхимо устанавливать непосредствено на виртульных или физических серверах,
           а не в докере, для получения корректных метрик от  ядра ОС.   
       Grafana - является системой визуализации данных, но не хранит их .
           Поэтому  данный сервис можно легко развернуть в докер-контейнере .
...
#### Gitlab сервер для реализации CI/CD процессов и приватный (закрытый) Docker Registry.
      Для организации Gitlab сервер лучше развернуть на виртуальной среде.
      А вот для gitlab-runnerы  идеально сработает  контейнеризация.
      В среде контейнеров плюсом для пользователей  будет безопасность и легкость масштабирования и развертывания  runner-сервисов.

     Для организации приватного репозитория (закрытый) Docker Registry контейнеризация  не подойдет. Причины:  
