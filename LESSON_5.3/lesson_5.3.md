...
#### Высоконагруженное монолитное java веб-приложение;
     - Рекомендую физический сервер.  
       ...
       Предполагается  высокая нагрузка  и отсутвие масштабировния -  значит необходим скоростной физический доступ к ресурсам (CPU и дискам).
       Виртуалзация с гипервизовром или физический сервер без использования слоя гипервизора.
---
...
#### Шина данных на базе Apache Kafka;
      Рпасреденнная потоковая платформа  для потсорения конвейцров в режиме рельаного времени,которая может испольшватетья как брокер сообщений.
    - Количество реализаций в в контенейнерах просто зашккаливает - значит  решение успешно .
...
#### Elasticsearch кластер для реализации логирования продуктивного веб-приложения - три ноды elasticsearch, два logstash и две ноды kibana;
    - Ноды Elasticsearch лучше развернуть на виртуальных хостах внутри отказоустойчивого кластера, например VMWARE VCenter,
      хотя количестов редаизаций на Dokcer-образе  - огромно.
...
#### Gitlab сервер для реализации CI/CD процессов и приватный (закрытый) Docker Registry.
      ...
      А вот gitlab-runnerы  идельано сработает  контейнеризацуия .
